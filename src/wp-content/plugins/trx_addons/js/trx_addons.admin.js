...
(function() {
	...
	jQuery(document).ready(function() {
		...
		jQuery('body').on('change', 'select[name*="post_type"],select[name*="taxonomy"]', function () {
			var cat_fld = jQuery(this).parents('.siteorigin-widget-field').next().find('select');
			if (cat_fld.length > 0) {
				var cat_lbl = jQuery(this).parents('.siteorigin-widget-field').next().find('.siteorigin-widget-field-label');
		...
		jQuery('body').on('change', 'select.layout', function () {
			...
			if (a.length > 0 && id > 0)
				a.attr('href', a.attr('href').replace(/post=[0-9]{1,4}/, "post="+id));
		});
	...
	// Init SO Panels dialog
	// -----------------------------------------------------------------------------------
/*
	function trx_addons_admin_init_so_panels() {
		var dialog = jQuery( '.so-panels-dialog-wrapper:not(.inited)' ).addClass('inited');
		if (dialog.length > 0) {
			var view = dialog.data( 'view' );
			if ( ! _.isUndefined( view ) ) {
				setTimeout(function() { trx_addons_admin_init_fields(null, dialog); }, 500);
				view.on( 'open_dialog_complete', function() {
					trx_addons_admin_init_fields(null, dialog);
				});
			}
		}
		setTimeout(trx_addons_admin_init_so_panels, 500);
	}
*/
