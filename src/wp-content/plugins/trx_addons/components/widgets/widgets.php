...
if (!function_exists('trx_addons_widgets_show_field')) {
	function trx_addons_widgets_show_field($widget, $params=array()) {
		?><div class="widget_field_type_<?php echo esc_attr($params['type']); ?>"><?php
			if ($params['type'] == 'info') {
				?>
				<label><?php echo esc_html($params['label']); ?></label>
				<span><?php echo esc_html($params['value']); ?></span>
				<?php
			} else {
				?><label for="<?php echo esc_attr($widget->get_field_id($params['name'])); ?>"><?php echo esc_html($params['label']); ?></label><?php
			}
			if ($params['type'] == 'select') {
				?><select id="<?php echo esc_attr($widget->get_field_id($params['name'])); ?>"
						name="<?php echo esc_attr($widget->get_field_name($params['name'])); ?>"
				...
				if (is_array($params['options']) && count($params['options']) > 0) {
					...
					. '</option>';
						?><label><input type="radio"
									id="<?php echo esc_attr($widget->get_field_id($params['name']).'_'.$slug); ?>"
									name="<?php echo esc_attr($widget->get_field_name($params['name'])); ?>"
									value="<?php echo esc_attr($slug); ?>" <?php echo ($params['value']==$slug ? ' checked="checked"' : ''); ?> />
				...
				?><input type="text"
						id="<?php echo esc_attr($widget->get_field_id($params['name'])); ?>" 
						name="<?php echo esc_attr($widget->get_field_name($params['name'])); ?>"
				...
				trx_addons_show_layout(trx_addons_options_show_custom_field($widget->get_field_id($params['name']).'_button', 
								array(
									'type' => 'mediamanager',
									'multiple' => !empty($params['multiple']),
									'data_type' => $params['type'],
									'class_field' => !empty($params['class_field']) ? ' '.esc_attr($params['class_field']) : '',
									'linked_field_id' => $widget->get_field_id($params['name'])
									),
								$params['value']));
				...
				?><input type="hidden"
						...
						name="<?php echo esc_attr($widget->get_field_name($params['name'])); ?>"
						value="<?php echo esc_attr($params['value']); ?>" /><?php
				trx_addons_show_layout(trx_addons_options_show_custom_field('trx_addons_options_field_'.esc_attr($widget->get_field_id($params['name'])), 
								array(
									'type' => 'icons',
									'class_field' => !empty($params['class_field']) ? ' '.esc_attr($params['class_field']) : '',
				...
				?><textarea id="<?php echo esc_attr($widget->get_field_id($params['name'])); ?>" 
						name="<?php echo esc_attr($widget->get_field_name($params['name'])); ?>"
						class="widgets_param_fullwidth"><?php
