...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['actions'] as $item) {
		...
		?><div class="sc_action_item sc_action_item_<?php
			...
			if (!empty($item['height']) && (int)$args['full_height'] == 0) echo ' sc_action_fixed_height';
			if (!empty($item['position'])) echo ' sc_action_item_'.esc_attr($item['position']); 
			?>"<?php
			if (!empty($item['bg_image']) || !empty($item['bg_color']) || (!empty($item['height']) && (int)$args['full_height'] == 0)) 
				echo ' style="'
							...
							. (!empty($item['height']) && (int)$args['full_height'] == 0 ? 'height:'.esc_attr($item['height']).';overflow:hidden;' : '')
							. '"'; 
		?>>
			<?php
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_action_item_link_classes', 'sc_action_item_link sc_button sc_button_size_small')); ?>"><?php 
