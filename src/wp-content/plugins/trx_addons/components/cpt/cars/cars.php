...
if (!function_exists('trx_addons_cpt_cars_init')) {
	...
	function trx_addons_cpt_cars_init() {
		...
		$TRX_ADDONS_STORAGE['post_types'][] = TRX_ADDONS_CPT_CARS_PT;
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_CARS_PT] = array(
			"basic_section" => array(
			...
			"maker" => array(
				"title" => esc_html__("Maker", 'trx_addons'),
				"desc" => wp_kses_data( __("Select the car's maker", 'trx_addons') ),
			...
			"produced" => array(
				...
				"desc" => wp_kses_data( __('Select the year when the car is produced', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => date('Y'),
			...
			"price" => array(
				...
				"desc" => wp_kses_data( __('Specify main price for this car', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
			"price2" => array(
				...
				"desc" => wp_kses_data( __('Optional (rent) price', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
...
if ( !function_exists( 'trx_addons_cpt_cars_load_scripts_front' ) ) {
	...
	function trx_addons_cpt_cars_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-cpt_cars', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'cars/cars.css'), array(), null );
...
if ( !function_exists( 'trx_addons_cpt_cars_merge_styles' ) ) {
	add_action("trx_addons_filter_merge_styles", 'trx_addons_cpt_cars_merge_styles');
	function trx_addons_cpt_cars_merge_styles($list) {
		$list[] = TRX_ADDONS_PLUGIN_CPT . 'cars/cars.css';
		return $list;
	}
}
	...
	function trx_addons_cpt_cars_get_blog_all_posts_link($link='', $args=array()) {
		if ($link=='') {
			if (trx_addons_is_cars_page() 
				...
				$link = '<a href="'.esc_url(get_post_type_archive_link( TRX_ADDONS_CPT_CARS_PT )).'">'
						. esc_html__('All Cars', 'trx_addons')
						. '</a>';
		}
		return $link;
	}
		...
		if ( defined('TRX_ADDONS_CPT_CARS_PT') ) {
			if ( is_post_type_archive(TRX_ADDONS_CPT_CARS_PT) ) {
				if ( (int) trx_addons_get_value_gp('compare') == 1)
					...
					$title = esc_html__('All Cars', 'trx_addons');
			}
	...
	function trx_addons_cpt_cars_query_params_to_args($params=array(), $new_query=false) {
		...
		if (!empty($params['cars_mileage']))
			if ($params['cars_mileage']!=trx_addons_cpt_cars_get_min_max('mileage_min').','.trx_addons_cpt_cars_get_min_max('mileage_max'))
				$args = trx_addons_query_add_meta($args, 'trx_addons_cars_bedrooms', $params['cars_bedrooms']);
		...
		if (!empty($params['cars_order'])) {
			$order = explode('_', $params['cars_order']);
			if (count($order) == 1)
				$order[] = 'asc';
			if ($order[0] == 'price') {
				$args['meta_key'] = 'trx_addons_cars_price';
				$args['orderby'] = 'meta_value_num';
			} else if ($order[0] == 'title')
				$args['orderby'] = 'title';
	...
	function trx_addons_cpt_cars_pre_get_posts($query) {
		...
		if ($post_type == TRX_ADDONS_CPT_CARS_PT) {
			...
			// Filters and sort for the foreground lists
				...
				$args = trx_addons_cpt_cars_query_params_to_args();
...
if (!function_exists('trx_addons_sc_cars_add_in_vc_params')) {
	function trx_addons_sc_cars_add_in_vc_params() {
		...
		$params = array_merge(
				array(
					...
					array(
						...
						"heading" => esc_html__("Maker", 'trx_addons'),
						"description" => wp_kses_data( __("Select the car's maker", 'trx_addons') ),
						'edit_field_class' => 'vc_col-sm-4',
						"admin_label" => true,
				        'save_always' => true,
						"value" => array_merge(array(esc_html__('- Maker -', 'trx_addons') => 0), array_flip(trx_addons_get_list_terms(false, TRX_ADDONS_CPT_CARS_TAXONOMY_MAKER))),
						"std" => "0",
						"type" => "dropdown"
					),
					array(
		...
		$params = trx_addons_vc_add_param_option($params, 'orderby', array( 
																			"value" => array_flip(array_merge(trx_addons_get_list_sc_query_orderby(), array(
	...
	class TRX_Addons_SOW_Widget_Cars extends TRX_Addons_SOW_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('ThemeREX Addons SOW - Cars', 'trx_addons'),
				array(
		...
		function get_widget_form() {
			...
			return apply_filters('trx_addons_sow_map', array_merge(
				array(
					...
					"cars_maker" => array(
						"label" => esc_html__("Maker", 'trx_addons'),
						"description" => wp_kses_data( __("Select the car's maker", 'trx_addons') ),
						"default" => 0,
						"options" => trx_addons_array_merge(array(0 => esc_html__('- Maker -', 'trx_addons')), trx_addons_get_list_terms(false, TRX_ADDONS_CPT_CARS_TAXONOMY_MAKER)),
						"type" => "select"
					),
					"cars_model" => array(
				...
				trx_addons_sow_add_query_param('', array(
					'orderby' => array( 
									"options" => trx_addons_array_merge(trx_addons_get_list_sc_query_orderby(), array(
										'price' => esc_html__('Price', 'trx_addons')
									))
								),
					'columns' => array( 
