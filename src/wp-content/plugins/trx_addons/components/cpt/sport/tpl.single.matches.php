				...
				<div class="sport_page_meta">
					<span class="sport_page_meta_item sport_page_meta_date"><?php
						...
						echo sprintf($dt < date('Y-m-d') 
										...
										'<span class="sport_page_meta_item_date">' . date(get_option('date_format'), strtotime($dt)) . '</span>');
					?></span>
				</div>
				<?php
			}
		...
		?><div class="sport_page_lists match_page_lists"><?php
			...
			if (isset($meta['players'])) {
				...
				foreach ($meta['players'] as $id=>$v)
					if ((int)$v == 1) $players[$id] = !empty($meta['points']) && isset($points[$points_pos]) ? (float) $points[$points_pos++] : 0;
					...
					foreach ($matches as $id_match=>$name_match) {
						...
						if ($last_date != $meta['date_start']) {
							...
							?><li><?php
								...
								?><span class="sport_page_round_name"><?php echo date(get_option('date_format'), strtotime($meta['date_start'])); ?></span><?php
							?></li></a><?php
						}
						?><li><a href="<?php echo esc_url(get_permalink($id_match)); ?>" class="sport_page_list_link"><?php
							...
							if (isset($meta['player1'])) {
								...
								// Start time
								?><span class="sport_page_match_time"><?php echo esc_html(!empty($meta['time_start']) ? date(get_option('time_format'), strtotime($meta['time_start'])) : '--:--'); ?></span><?php
								// First player's name
