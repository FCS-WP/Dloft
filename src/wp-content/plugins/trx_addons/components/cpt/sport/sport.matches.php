...
if (!function_exists('trx_addons_cpt_matches_init')) {
	...
	function trx_addons_cpt_matches_init() {
		...
		$TRX_ADDONS_STORAGE['post_types'][] = TRX_ADDONS_CPT_MATCHES_PT;
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT] = array(
			"date_start" => array(
		...
		if (!empty($competition_meta['type']) && $competition_meta['type'] == 'group') {
			unset($TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT]['player1']);
			unset($TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT]['player2']);
			unset($TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT]['score']);
			if (isset($players['none'])) unset($players['none']);
			trx_addons_array_insert_before($TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT], 'points', array(
				"players" => array(
			...
			$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT]['points']['type'] = 'textarea';
			$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_MATCHES_PT]['points']['desc'] = wp_kses_data( __("Points, credited to players, based on the results of the match. Each value must be on new line! Number lines must be equivalent to the number players", 'trx_addons') );
		}
			...
			if (trx_addons_is_matches_page()) {
				...
				if ($round_post) {
					...
					if (($competition = $round_post->post_parent) > 0) {
						...
						if ($sport)
							$new_links = '<a href="'.esc_url(get_post_type_archive_link( TRX_ADDONS_CPT_COMPETITIONS_PT )).'">'.esc_html__('All Competitions', 'trx_addons').'</a>'
								. $args['delimiter']
...
if ( !function_exists( 'trx_addons_cpt_matches_get_blog_title' ) ) {
	...
	function trx_addons_cpt_matches_get_blog_title($title='') {
		if ( is_post_type_archive(TRX_ADDONS_CPT_MATCHES_PT) ) {
			$round = trx_addons_get_value_gp('round');
			$title = (int) $round > 0 ? get_the_title($round) : esc_html__('All Matches', 'trx_addons');
		}
		return $title;
	}
}
		...
		if ($column_name == 'cpt_matches_date') {
			...
			if (!empty($meta['date_start'])) {
				?><div class="trx_addons_meta_row">
					...
					<span class="trx_addons_meta_data"><?php echo date(get_option('date_format').' '.get_option('time_format'), strtotime($meta['date_start'].' '.$meta['time_start'])); ?></span>
				</div><?php
			}
			if (!empty($meta['place'])) {
				?><div class="trx_addons_meta_row">
