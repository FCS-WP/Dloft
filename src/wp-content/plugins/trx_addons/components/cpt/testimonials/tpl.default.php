	...
	?><div class="sc_testimonials sc_testimonials_<?php
		...
		if ($args['slider']) {
			...
			if (!empty($args['slider_pagination_thumbs'])) {
				...
				$no_image = apply_filters('trx_addons_filter_no_image', trx_addons_get_file_url('css/images/no-avatar.png'));
			}
		...
		while ( $query->have_posts() ) { $query->the_post();
			if ($args['slider'] && !empty($args['slider_pagination_thumbs'])) {
				...
				$pagination_bullets .= '<span class="swiper-pagination-button"'
