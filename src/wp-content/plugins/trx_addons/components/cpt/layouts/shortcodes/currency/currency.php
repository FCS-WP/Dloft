	...
	function trx_addons_sc_layouts_currency_load_scripts_front() {
		if (function_exists('trx_addons_exists_woocommerce') && trx_addons_exists_woocommerce() && class_exists('WOOCS')) {
			if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
				wp_enqueue_style( 'trx_addons-sc_layouts_currency', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'currency/currency.css'), array(), null );
			}
		}
	}
}
...
if ( !function_exists( 'trx_addons_sc_layouts_currency_merge_styles' ) ) {
	...
	function trx_addons_sc_layouts_currency_merge_styles($list) {
		$list[] = TRX_ADDONS_PLUGIN_CPT_LAYOUTS_SHORTCODES . 'currency/currency.css';
		return $list;
	}
}
	...
	class TRX_Addons_SOW_Widget_Layouts_Currency extends TRX_Addons_SOW_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('ThemeREX Addons SOW - Layouts: Currency', 'trx_addons'),
				array(
