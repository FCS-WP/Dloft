	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( 'team_member_page itemscope' ); trx_addons_seo_snippets('', 'Article'); ?>>
		...
		?><div class="team_member_description"><?php
			...
			<div class="team_member_details">
				<?php
				$meta_box = apply_filters('trx_addons_filter_meta_box_fields', $TRX_ADDONS_STORAGE['meta_box_'.get_post_type()], get_post_type());
				foreach ($meta_box as $k=>$v) {
					if (!empty($v['details']) && !empty($meta[$k])) {
						?><div class="team_member_details_<?php echo esc_attr($k); ?>"><span class="team_member_details_label"><?php echo esc_html($v['title']); ?>: </span><span class="team_member_details_value"><a href="mailto:i<?php echo esc_html($meta[$k]); ?>"><?php echo esc_html($meta[$k]); ?></a></span></div><?php
					}
				}
				?>
			</div>
