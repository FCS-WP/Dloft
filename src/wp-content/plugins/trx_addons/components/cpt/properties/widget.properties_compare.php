...
// Widget Class
class trx_addons_widget_properties_compare extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_properties_compare', 'description' => esc_html__('Compare selected properties', 'trx_addons'));
		parent::__construct( 'trx_addons_widget_properties_compare', esc_html__('ThemeREX Addons - Properties Compare', 'trx_addons'), $widget_ops );
	}
	...
	function trx_addons_sc_widget_properties_compare($atts, $content=null){	
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '>';
			ob_start();
			the_widget( $type, $atts, trx_addons_prepare_widgets_args($TRX_ADDONS_STORAGE['widgets_args'], $id ? $id.'_widget' : 'widget_properties_compare', 'widget_properties_compare') );
