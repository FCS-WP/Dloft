...
if (!function_exists('trx_addons_cpt_properties_init')) {
	...
	function trx_addons_cpt_properties_init() {
		...
		$TRX_ADDONS_STORAGE['post_types'][] = TRX_ADDONS_CPT_PROPERTIES_PT;
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_PROPERTIES_PT] = array(
			"basic_section" => array(
			...
			"price" => array(
				...
				"desc" => wp_kses_data( __('Specify main price for this property', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
			"price2" => array(
				...
				"desc" => wp_kses_data( __('Optional price for rental or square feet/m', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
			...
			"bedrooms" => array(
				...
				"desc" => wp_kses_data( __('Bedrooms number (digits only)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => 0,
			...
			"bathrooms" => array(
				...
				"desc" => wp_kses_data( __('Bathrooms number (digits only)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => 0,
				"options" => trx_addons_get_list_range(0, 10),
				"type" => "select"
			...
			"garages" => array(
				...
				"desc" => wp_kses_data( __('Garages number (digits only)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => 0,
				"options" => trx_addons_get_list_range(0, 10),
				"type" => "select"
			...
			"built" => array(
				...
				"desc" => wp_kses_data( __('Specify or select the year when the property is built', 'trx_addons') ),
...
if ( !function_exists( 'trx_addons_cpt_properties_load_scripts_front' ) ) {
	...
	function trx_addons_cpt_properties_load_scripts_front() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-cpt_properties', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'properties/properties.css'), array(), null );
...
if ( !function_exists( 'trx_addons_cpt_properties_merge_styles' ) ) {
	add_action("trx_addons_filter_merge_styles", 'trx_addons_cpt_properties_merge_styles');
	function trx_addons_cpt_properties_merge_styles($list) {
		$list[] = TRX_ADDONS_PLUGIN_CPT . 'properties/properties.css';
		return $list;
	}
}
	...
	function trx_addons_cpt_properties_get_blog_all_posts_link($link='', $args=array()) {
		if ($link=='') {
			if (trx_addons_is_properties_page() 
				...
				$link = '<a href="'.esc_url(get_post_type_archive_link( TRX_ADDONS_CPT_PROPERTIES_PT )).'">'
						. esc_html__('All Properties', 'trx_addons')
						. '</a>';
		}
		return $link;
	}
		...
		if ( defined('TRX_ADDONS_CPT_PROPERTIES_PT') ) {
			if ( is_post_type_archive(TRX_ADDONS_CPT_PROPERTIES_PT) ) {
				if ( (int) trx_addons_get_value_gp('compare') == 1)
					...
					$title = esc_html__('All Properties', 'trx_addons');
			}
...
if ( !function_exists( 'trx_addons_cpt_properties_query_params_to_args' ) ) {
	function trx_addons_cpt_properties_query_params_to_args($params=array(), $new_query=false) {
		...
		if (!empty($params['properties_order'])) {
			$order = explode('_', $params['properties_order']);
			if (count($order) == 1)
				$order[] = 'asc';
			if ($order[0] == 'price') {
	...
	class TRX_Addons_SOW_Widget_Properties extends TRX_Addons_SOW_Widget {
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('ThemeREX Addons SOW - Properties', 'trx_addons'),
				array(
