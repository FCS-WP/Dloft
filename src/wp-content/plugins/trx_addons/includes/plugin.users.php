...
if (!function_exists('trx_addons_add_login_link')) {
	...
	function trx_addons_add_login_link($login_text='', $logout_text='') {
		global $TRX_ADDONS_STORAGE;
		$TRX_ADDONS_STORAGE['login_popup'] = !is_user_logged_in();
		trx_addons_get_template_part('templates/tpl.login-link.php',
										'trx_addons_args_login',
										array(
											'login_text' => empty($login_text) ? esc_html__('Login or|Register', 'trx_addons') : $login_text,
											'logout_text' => empty($logout_text) ? esc_html__('Logout|%s', 'trx_addons') : $logout_text
											)
									);
	}
}
