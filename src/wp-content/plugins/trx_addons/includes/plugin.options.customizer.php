	...
	function trx_addons_options_show_field($name, $field, $group='') {
		...
		// Type 'select', 'select2', 'post_type', 'taxonomy'
			...
			if ($field['type']=='select2') {
				...
				$field['options'] = array();
				if (!empty($last_post_type)) {
					$taxonomies = get_object_taxonomies($last_post_type, 'objects');
					foreach ($taxonomies as $slug=>$taxonomy_obj)
						$field['options'][$slug] = $taxonomy_obj->label;
				}
		...
		switch ($field['type']) {
			...
			case 'icons':
				if (is_array($field['options']) && count($field['options']) > 0) {
					...
					$output .= '<span class="trx_addons_icon_selector'
												. (!empty($field['class_field']) ? ' '.esc_attr($field['class_field']) : '') 
												. ($socials_type=='icons' && !empty($value) ? ' '.esc_attr($value) : '')
												. '"'
										. ' title="'.esc_attr__('Select icon', 'trx_addons').'"'
										. ' data-style="'.($socials_type=='images' ? 'images' : 'icons').'"'
										. ($socials_type=='images' && !empty($value) 
