...
$css_image =   (!empty($args['image_bg_color']) ? 'background-color:' . esc_attr($args['image_bg_color']) . ';' : '')
			 ...
			 . (!empty($args['image']) ? 'width:'.trim($args['image_width']).';' : '')
...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><div class="sc_promo_image" style="<?php echo esc_attr($css_image); ?>"><?php
			...
			// Button on the image
            if (function_exists('trx_addons_sc_button') && !empty($args['link_image_url']) && !empty($args['link_text_image']) ) {
                ?><div class="sc_promo_button_on_image"><?php
                palladio_show_layout(trx_addons_sc_button(array(
                            'type' => 'simple',
                            'title' => $args['link_text_image'],
                            'link' => $args['link_image_url'],
                            'class' => 'sc_item_button sc_item_button_simple sc_promo_button',
                            'align' => 'none'
                        )
                    )
                );
