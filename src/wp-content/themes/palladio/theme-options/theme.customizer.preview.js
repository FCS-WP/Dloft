...
( function( $ ) {
	...
	var $style = $('#palladio-color-scheme-css'),
		api = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length == 0 ) {
		$style = $('head').append( '<'+'style type="text/css" id="palladio-color-scheme-css" />' ).find('#palladio-color-scheme-css');
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id == 'body_style') {
				...
				$('.single-product .trx-stretch-width-wrap').removeClass('scheme_default scheme_light scheme_dark scheme_black').addClass('scheme_'+val);
				...
				$('.sc_layouts_logo .logo_text').html( palladio_prepare_macros(val) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( palladio_prepare_macros(val) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( palladio_prepare_macros(val) );
