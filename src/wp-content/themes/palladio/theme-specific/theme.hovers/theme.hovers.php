...
if (!function_exists('palladio_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'palladio_hovers_theme_setup3', 3 );
	function palladio_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		palladio_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'palladio'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'palladio') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'palladio'),
					'slide_left'	=> esc_html__('Slide from Left',	'palladio'),
					'slide_right'	=> esc_html__('Slide from Right',	'palladio'),
					'slide_top'		=> esc_html__('Slide from Top',		'palladio'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'palladio')
				),
				"type" => "hidden"//"select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'palladio'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'palladio') ),
				"std" => 'icons',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'palladio'),
					'icon'	=> esc_html__('Icon',	'palladio'),
					'icons'	=> esc_html__('Icons',	'palladio')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('palladio_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'palladio_hovers_theme_setup9', 9 );
	function palladio_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'palladio_hovers_frontend_scripts', 1010 );
		add_filter( 'palladio_filter_localize_script','palladio_hovers_localize_script' );
		add_filter( 'palladio_filter_merge_scripts',	'palladio_hovers_merge_scripts' );
		add_filter( 'palladio_filter_merge_styles',	'palladio_hovers_merge_styles' );
		add_filter( 'palladio_filter_get_css', 		'palladio_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'palladio_hovers_frontend_scripts' ) ) {
	
	function palladio_hovers_frontend_scripts() {
		if ( palladio_is_on(palladio_get_theme_option('debug_mode')) && palladio_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'palladio-hovers', palladio_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( palladio_is_on(palladio_get_theme_option('debug_mode')) && palladio_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'palladio-hovers',  palladio_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('palladio_hovers_merge_scripts')) {
	
	function palladio_hovers_merge_scripts($list) {
...
if (!function_exists('palladio_hovers_merge_styles')) {
	
	function palladio_hovers_merge_styles($list) {
...
if (!function_exists('palladio_hovers_localize_script')) {
	
	function palladio_hovers_localize_script($arr) {
		$arr['button_hover'] = palladio_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('palladio_hovers_add_icons') ) {
	function palladio_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'palladio') : '')
										. '</a>',
										$product );
				}?>
			</div>
...
// Add styles into CSS
if ( !function_exists( 'palladio_hovers_get_css' ) ) {
	
	function palladio_hovers_get_css($css, $colors, $fonts, $scheme='') {
