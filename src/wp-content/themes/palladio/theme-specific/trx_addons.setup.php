...
------------------------------------------------------------------------------- */
if (!function_exists('palladio_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'palladio_trx_addons_theme_specific_components');
	function palladio_trx_addons_theme_specific_components($enable=false) {
		return PALLADIO_THEME_FREE
					? false		// Free version
...
if (!function_exists('palladio_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'palladio_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'palladio_trx_addons_theme_specific_setup1', 1 );
	function palladio_trx_addons_theme_specific_setup1() {
		if (palladio_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'palladio_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'palladio_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'palladio_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'palladio_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'palladio_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'palladio_trx_addons_widgets_list');

            add_filter('trx_addons_filter_sc_price_item_link_classes', function () {
                return 'sc_price_item_link sc_button sc_button_simple';
            });
        }
	}
}
...
// CV
if ( !function_exists( 'palladio_trx_addons_cv_enable' ) ) {
	
	function palladio_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'palladio_trx_addons_demo_enable' ) ) {
	
	function palladio_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'palladio_trx_addons_edd_themes_market_enable' ) ) {
	
	function palladio_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'palladio_trx_addons_api_list' ) ) {
	
	function palladio_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'palladio_trx_addons_cpt_list' ) ) {
	
	function palladio_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'palladio_trx_addons_sc_list' ) ) {
	
	function palladio_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'palladio_trx_addons_widgets_list' ) ) {
	
	function palladio_trx_addons_widgets_list($list=array()) {
        ...
        // Param 'controls_style' for trx_addons_widgets_list
        $list['slider']['layouts_sc'] = array(
            'default' => esc_html__('Default', 'palladio'),
            'modern' => esc_html__('Modern', 'palladio'),
            'dark' => esc_html__('Dark', 'palladio'),
        );
...
if ( !function_exists( 'palladio_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'palladio_trx_addons_menu_cache');
	function palladio_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('palladio_trx_addons_localize_script')) {
	...
	function palladio_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = palladio_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'palladio_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'palladio_trx_addons_sc_type', 10, 2);
	function palladio_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
        if (in_array($sc, array('trx_sc_title')))
            $list['row'] = esc_html__('Row', 'palladio');
        if (in_array($sc, array('trx_sc_button')))
            $list['simple_line'] = esc_html__('Simple with Line', 'palladio');
		return $list;
	}
}

// Add params to the default shortcode's atts
if ( !function_exists( 'palladio_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'palladio_trx_addons_sc_atts', 10, 2);
	function palladio_trx_addons_sc_atts($atts, $sc) {
		
		// Param 'scheme'
		if (in_array($sc, array('trx_sc_promo'))) {
            $atts['align_text'] = false;
            $atts['link_image_url'] = '';
            $atts['link_text_image'] = '';
        }
...
if ( !function_exists( 'palladio_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'palladio_trx_addons_sc_map', 10, 2);
	function palladio_trx_addons_sc_map($params, $sc) {

        // Param 'align_text' for trx_slider_promo
        if (in_array($sc, array('trx_sc_promo'))) {
            $params['params'][] = array(
                'param_name' => 'align_text',
                'heading' => esc_html__('Align Text', 'palladio'),
                'description' => wp_kses_data(__('Align text to the width of the content. It works if the row is stretched to the full width.', 'palladio')),
                'admin_label' => true,
                'value' => array(
                    esc_html__("Yes", 'palladio') => true,
                    esc_html__("No", 'palladio') => false
                ),
                'type' => 'dropdown'
            );
            $params['params'][] = array(
                "param_name" => "link_image_url",
                "heading" => esc_html__("Button URL", 'palladio'),
                "description" => wp_kses_data(__("Link URL for the button at the image", 'palladio')),
                'group' => esc_html__('Image', 'palladio'),
                'edit_field_class' => 'vc_col-sm-4',
                "type" => "textfield"
            );
            $params['params'][] = array(
                "param_name" => "link_text_image",
                "heading" => esc_html__("Button's text", 'palladio'),
                "description" => wp_kses_data(__("Caption for the button at the image", 'palladio')),
                'group' => esc_html__('Image', 'palladio'),
                'edit_field_class' => 'vc_col-sm-4',
                "type" => "textfield"
            );
        ...
        // Param 'controls_style' for trx_slider_controls
        if (in_array($sc, array('trx_slider_controls'))) {
            $params['params'][1]['value'] = array(
                esc_html__('Default', 'palladio') => 'default',
                esc_html__('Dark', 'palladio') => 'dark'
            );
        }
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'palladio'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'palladio') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(palladio_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
		if (in_array($sc, array('trx_sc_button'))) {
			if (empty($params['params']) || !is_array($params['params'])) $params['params'] = array();
...
if ( !function_exists( 'palladio_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'palladio_trx_addons_sow_map', 10, 2);
	function palladio_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'palladio'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'palladio') ),
				'options' => palladio_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			palladio_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			palladio_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'palladio_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'palladio_trx_addons_sc_output', 10, 4);
	function palladio_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !palladio_is_inherit($atts['color_style']))
			...
			if (!empty($atts['color_style']) && !palladio_is_inherit($atts['color_style']))
				$output = str_replace('class="sc_promo ', 'class="sc_promo color_style_'.esc_attr($atts['color_style']).' ', $output);if (!empty($atts['color_style']) && !palladio_is_inherit($atts['color_style']))
			if (!empty($atts['align_text']) && !palladio_is_inherit($atts['align_text']))
				$output = str_replace('class="sc_promo ', 'class="sc_promo align_text ', $output);
...
if ( !function_exists( 'palladio_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'palladio_trx_addons_sc_item_title_tag');
	function palladio_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'palladio_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'palladio_trx_addons_sc_item_button_args', 10, 3);
	function palladio_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'palladio_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'palladio_trx_addons_slider_title', 10, 2 );
	function palladio_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'palladio_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'palladio_trx_addons_sc_googlemap_styles');
	function palladio_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'palladio');
		$list['light'] = esc_html__('Light', 'palladio');
		return $list;
	}
}
...
if ( !function_exists( 'palladio_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'palladio_trx_addons_editor_init', 11);
	function palladio_trx_addons_editor_init($opt) {
		if (palladio_exists_trx_addons()) {
			...
			if (false && !empty($opt['style_formats'])) {
				...
				if (is_array($style_formats) && count($style_formats)>0 ) {
					foreach ($style_formats as $k=>$v) {
						if ( $v['title'] == esc_html__('List styles', 'palladio') ) {
							$style_formats[$k]['items'][] = array(
										'title' => esc_html__('Arrow', 'palladio'),
										'selector' => 'ul',
...
if ( !function_exists( 'palladio_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'palladio_filter_allow_override_header_image', 'palladio_trx_addons_allow_override_header_image' );
	function palladio_trx_addons_allow_override_header_image($allow) {
		return palladio_is_team_page() || palladio_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'palladio_trx_addons_sidebar_present' ) ) {
	add_filter( 'palladio_filter_sidebar_present', 'palladio_trx_addons_sidebar_present' );
	function palladio_trx_addons_sidebar_present($present) {
		return !is_single() && (palladio_is_team_page() || palladio_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'palladio_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'palladio_trx_addons_thumb_size', 10, 2);
	function palladio_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = palladio_get_thumb_size('big');
		return $thumb_size;
	}
}
...
if (!function_exists('palladio_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'palladio_trx_addons_override_options_fields', 10, 2);
	function palladio_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'palladio'),
				"desc" => wp_kses_data( __("Team member's email", 'palladio') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
